...
int main() {
    ...
    scanf("%s", search);
    int foundRideNumber = search_rideNumber(trains2, SIZE, search);
    if (foundRideNumber != -1) {
        printf("По направлению %s найден номер поезда: %d\n", search, foundRideNumber);
    }
    else {
        printf("По направлению %s поезд не найден.\n", search);
    }

    qsort(trains2, SIZE, sizeof(train_t), compare);
    print_array(trains2, SIZE);
